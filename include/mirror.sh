#!/bin/bash
Pcre_Ver='pcre-8.37.tar.gz'
Pcre_Mirror='http://sourceforge.net/projects/pcre/files/pcre/8.37/pcre-8.37.tar.gz/download'

Libiconv_Ver='libiconv-1.14.tar.gz'
Libiconv_Mirror='http://ftp.gnu.org/gnu/libiconv/libiconv-1.14.tar.gz'

LibMcrypt_Ver='libmcrypt-2.5.8.tar.gz'
LibMcrypt_Mirror='http://sourceforge.net/projects/mcrypt/files/Libmcrypt/2.5.8/libmcrypt-2.5.8.tar.gz/download'


Mcypt_Ver='mcrypt-2.6.8.tar.gz'
Mcypt_Mirror='http://sourceforge.net/projects/mcrypt/files/MCrypt/2.6.8/mcrypt-2.6.8.tar.gz/download'

Php_Ver='php-5.6.15.tar.gz'
Php_Mirror='http://cn2.php.net/get/php-5.6.15.tar.gz/from/this/mirror'

Autoconf_Ver='autoconf-2.13.tar.gz'
Autoconf_Mirror='http://ftp.gnu.org/gnu/autoconf/autoconf-2.13.tar.gz'

Mash_Ver='mhash-0.9.9.9.tar.gz'
Mash_Mirror='http://downloads.sourceforge.net/mhash/mhash-0.9.9.9.tar.gz'

Nginx_Ver='nginx-1.8.0'
Nginx_Mirror='http://nginx.org/download/nginx-1.8.0.tar.gz'

Mysql_Ver='mysql-5.5.42'
Mysql_Mirror='http://dev.mysql.com/get/Downloads/MySQL-5.6/mysql-5.6.12.tar.gz'


Libmemcached_Ver='libmemcached-1.0.18'
Libmemcached_Mirror='https://launchpad.net/libmemcached/1.0/1.0.18/+download/libmemcached-1.0.18.tar.gz'

PHPMemcached_Ver='memcached-2.2.0'
PHPMemcached_Mirror='http://pecl.php.net/get/memcached-2.2.0.tgz'

Memcached_Ver='memcached-1.4.24.tar.gz'
Memcached_Mirror='http://memcached.org/files/memcached-1.4.24.tar.gz'

ImageMagick_Ver='ImageMagick-6.9.2-5.tar.gz'
ImageMagick_Mirror='http://mirror.checkdomain.de/imagemagick/ImageMagick-6.9.2-5.tar.gz'

Imagick_Ver='imagick-3.1.2'
Imagick_Mirror='http://pecl.php.net/get/imagick-3.1.2.tgz'

Freetype_Ver='freetype-2.4.12'
Curl_Ver='curl-7.42.1'
Jemalloc_Ver='jemalloc-3.6.0'
TCMalloc_Ver='gperftools-2.4'
Libunwind_Ver='libunwind-1.1'
XCache_Ver='xcache-3.2.0'
Redis_Stable_Ver='redis-3.0.1'
PHPRedis_Ver='redis-2.2.7'
